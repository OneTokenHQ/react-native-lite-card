package so.onetoken.app.wallet.lite
...
import so.onetoken.app.wallet.lite.nfc.NFCExceptions
import so.onetoken.app.wallet.lite.nfc.NfcUtils
import so.onetoken.app.wallet.lite.nfc.broadcast.NfcStatusChangeBroadcastReceiver
import so.onetoken.app.wallet.lite.onetokenLite.OneTokenLiteCard
import so.onetoken.app.wallet.lite.onetokenLite.entitys.CardState
import so.onetoken.app.wallet.lite.utils.NfcPermissionUtils
import so.onetoken.app.wallet.lite.utils.Utils
...
//                            mNFCConnectedChannel.receive()
                            val startRequest = OneTokenLiteCard.initRequest(isoDep)
                            val dataMap = Arguments.createMap().apply {
                                putInt("code", -1)
                                putString("type", "OneToken_Lite")
    ...
    override fun initialize() {
        ...
        Utils.getTopActivity()?.let {
            launch(Dispatchers.IO) {
                OneTokenLiteCard.startNfc(it as FragmentActivity) {}
            }
        }
    ...
    override fun onHostPause() {
        Utils.getTopActivity()?.let {
            launch(Dispatchers.IO) {
                try {
                    OneTokenLiteCard.stopNfc(it as FragmentActivity)
    ...
    private suspend fun acquireDevice(): IsoDep? {
        ...
        if (receiveIsoDep == null) {
            ...
            val initChannelRequest = OneTokenLiteCard.initRequest(receiveIsoDep)
    ...
    fun getLiteInfo(callback: Callback) {
        launch {
            ...
            handleOperation(callback) { isoDep ->
                ...
                val cardInfo = OneTokenLiteCard.getCardInfo(isoDep)
    ...
    fun setMnemonic(mnemonic: String, pwd: String, overwrite: Boolean, callback: Callback) {
        launch {
            handleOperation(callback) { isoDep ->
                ...
                val isSuccess =
                    OneTokenLiteCard.setMnemonic(mCurrentCardState, isoDep, mnemonic, pwd, overwrite)
    ...
    fun getMnemonicWithPin(pwd: String, callback: Callback) {
        launch {
            ...
            handleOperation(callback) { isoDep ->
                ...
                OneTokenLiteCard.getMnemonicWithPin(mCurrentCardState, isoDep, pwd)
            }
        }
    }
    ...
    fun changePin(oldPwd: String, newPwd: String, callback: Callback) {
        launch {
            ...
            handleOperation(callback) { isoDep ->
                ...
                OneTokenLiteCard.changPin(mCurrentCardState, isoDep, oldPwd, newPwd)
            }
        }
    }
    ...
    fun reset(callback: Callback) {
        launch {
            ...
            handleOperation(callback) { isoDep ->
                ...
                val isSuccess = OneTokenLiteCard.reset(isoDep)
                if (!isSuccess) throw NFCExceptions.ExecFailureException()
